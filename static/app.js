...
function buildOption() {
  const nodes = agents.map(a => ({
    ...
    symbolSize:   a.is_human ? 64 : 50,
    symbol:       a.avatar ? `image://${a.avatar}` : `image:///static/avatars/${a.id}.png`,
    label: {
      show: true,
...
function handleEvent(event) {
  switch (event.type) {
    ...
    case "init":
      ...
      edges  = [];
      if (event.max_ticks) maxTicks = event.max_ticks;
      renderGraph();
      log("🌏", "系统", `世界议题已投放 → ${event.topic}`, "#7c3aed");
      log("🔬", "系统", `本轮演化 ${maxTicks} 回合（根据议题复杂度自动调整）`, "#7c3aed");
      break;
    ...
    case "tick_start":
      ...
      document.getElementById("tick-label").innerHTML =
        `时钟: <span class="tick-value">${event.tick} / ${event.max_ticks}</span>`;
      break;

    case "agent_swap":
      // 动态换人：更新 agents 列表
      const swapIdx = agents.findIndex(a => a.id === event.left_id);
      if (swapIdx !== -1 && event.joined) {
        agents[swapIdx] = { ...event.joined, last_speech: "" };
      }
      renderGraph();
      log("🔄", "系统",
          `${event.left_name} 离开培养皿 → ${event.joined.name} 加入讨论！`,
          "#f59e0b");
      break;

    case "agent_added":
      if (event.joined) {
        agents.push({ ...event.joined, last_speech: "" });
      }
      renderGraph();
      log("🎉", "系统",
          `新思想者 ${event.joined.name} 加入了对局！`,
          "#34d399");
      break;
...
async function resetSandbox() {
  ...
  document.getElementById("tick-label").innerHTML = `时钟: <span class="tick-value">0 / 5</span>`;
  document.getElementById("btn-start").disabled = false;
...
document.addEventListener("DOMContentLoaded", async () => {
  initChart();
  log("🧫", "系统", "Petri 培养皿已就绪，等待演化指令...", "#34d399");
